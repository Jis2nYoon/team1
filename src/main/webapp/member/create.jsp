...
<script type="text/javascript">
  $(function() { // 자동 실행
    ...
    $.cookie('checkNICKNAME', 'FALSE'); // Cookie 생성
    $('#popup').hide();
  });
...
//이메일 정규표현식 검사
  function checkEmailStyle(myValue) {        
      var email = myValue;
    ...
    $.ajax({
      ...
      success: function(data){
        ...
        if (data.count > 0) {
          ...
          msg = "『Danger!』 이미 사용중인 이메일 입니다.";
        } else {
          if (checkEmailStyle(email) == false){
            $('#modal_content').attr('class', 'alert alert-danger'); // Bootstrap
            msg = "『Danger!』 이메일형식이 올바르지 않습니다.";
          }else{
            $('#modal_content').attr('class', 'alert alert-success'); // Bootstrap
            msg = "『Success!』 사용 가능한 이메일 입니다.";
         
            $.cookie('checkEMAIL', 'TRUE'); // Cookie 값 변경
          }
        }

        $('#modal_title').html('이메일 확인');
  ...
  function checkNickname(){
    var frm = $('#frm');
    var params = 'nickname='+$('#nickname', frm).val(); // #: id
    // alert('params: ' + params);
    ...
    $.ajax({
      ...
      success: function(data){
        ...
        if (data.count > 0) {
          $('#modal_content').attr('class', 'alert alert-danger'); // Bootstrap
          msg = "『Danger!』 이미 사용중인 닉네임 입니다.";
        } else {
          $('#modal_content').attr('class', 'alert alert-success'); // Bootstrap
          msg = "『Success!』 사용 가능한 닉네임 입니다.";
  ...
  function send() {
    var checkEMAIL = $.cookie('checkEMAIL'); // 쿠키값
    var checkNICKNAME = $.cookie('checkNICKNAME'); // 쿠키값
    
    
    if (checkEMAIL != 'TRUE') {
      var msg = "이메일 중복확인이 되지 않았습니다.<br>";
      msg += "이메일 [중복확인] 버튼을 클릭하세요.<br>";

      $('#modal_title').html('이메일 체크 확인');
      $('#modal_content').attr('class', 'alert alert-danger');
      $('#modal_content').html(msg);
      $('#modal_panel').modal(); // 다이얼로그 출력   
    ...
    if (checkNICKNAME != 'TRUE') {
      var msg = "닉네임 중복확인이 되지 않았습니다.<br>";
      msg += "닉네임 [중복확인] 버튼을 클릭하세요.<br>";
...
<DIV class='content' > 
  ...
  <FORM name='frm' id='frm' method='POST' action='./create.do' 
    ...
    <div class="form-group">
      ...
      <div class="col-md-10">
        <input type='text' class="form-control input-md" name='email' id='email' onchange="changeEmail()"  value='user@mail.com' required="required" style='width: 40%;' placeholder="이메일" autofocus="autofocus">
        <button type='button' onclick="checkEmail()" class="btn btn-info btn-md">중복확인</button>
    ...
    <div class="form-group">
      ...
      <div class="col-md-10">
        <input type='text' class="form-control input-md" name='nickname' id='nickname'  onchange="changeNickname()" value='왕누니' required="required" style='width: 30%;' placeholder="닉네임" autofocus="autofocus">
        <button type='button' onclick="checkNickname()"  class="btn btn-info btn-md">중복확인</button>
        <SPAN id='id_span'></SPAN> <!-- ID 중복 관련 메시지 -->        
      </div>
    </div>   
