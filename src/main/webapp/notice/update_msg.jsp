    ...
    <div style='text-align: center;'>
      <c:choose>
        <c:when test="${param.count == 1 }">
          <label>공지사항을 수정했습니다.</label><br>
        </c:when>
        <c:otherwise>
          <label>공지사항 수정에 실패했습니다.</label><br>
        </c:otherwise>
      </c:choose>
        <br>
        <c:if test="${param.count != 1 }">
          ...
          <button type="button" class="btn btn-default" onclick="location.href='./read.do?noticeno=${param.noticeno }&word=${param.word}&nowPage=${param.nowPage}'">조회</button>
