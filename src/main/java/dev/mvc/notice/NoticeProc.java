...
public class NoticeProc implements NoticeProcInter {
  ...
  public ArrayList<Mem_NoticeVO> list() {
    ArrayList<Mem_NoticeVO> list = noticeDAO.list();
    return list;
  }

  @Override
  public Mem_NoticeVO read(int noticeno) {
    Mem_NoticeVO mem_NoticeVO = noticeDAO.read(noticeno);
    return mem_NoticeVO;
  }
...
/*  @Override
  public ArrayList<NoticeVO> list_by_search(HashMap map) {
    ArrayList<NoticeVO> list = noticeDAO.list_by_search(map);
    
    int count = list.size();
    for (int i=0; i < count; i++) {
      NoticeVO noticeVO = list.get(i); // 흠,,,,,,
    }
    return list;
  }*/
  ...
  public String pagingBox(String listFile, int search_count, int nowPage, String word){ 
    ...
    str.append("    text-align: center;");    
    str.append("    background-color: #808080;"); 
...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
  ...
  public ArrayList<Mem_NoticeVO> list_by_search_paging(HashMap<String, Object> map) {
   ...
   map.put("endNum", endNum);
    ...
    ArrayList<Mem_NoticeVO> list = noticeDAO.list_by_search_paging(map);
    
    int count = list.size();
    for (int i=0; i < count; i++) {
      Mem_NoticeVO mem_NoticeVO = list.get(i); // 흠...!
    }
